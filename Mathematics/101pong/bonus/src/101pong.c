...
void pingpong(char **argv)
{
    ...
    printf("(%.2f, %.2f, %.2f)\n", x, y, z);
    printf("At time t + %s and at speed %s, ball coordinates will be:\n", argv[7], argv[8]);
    printf("(%.2f,  ", xx = ((my_atoi_mod(argv[1]) + (x * (my_atoi_mod(argv[7]) + 1)))) * my_atoi_mod(argv[8]));
    printf("%.2f, ", yy = ((my_atoi_mod(argv[2]) + (y * (my_atoi_mod(argv[7]) + 1)))) * my_atoi_mod(argv[8]));
    printf("%.2f)\n", zz = ((my_atoi_mod(argv[3]) + (z * (my_atoi_mod(argv[7]) + 1)))) * my_atoi_mod(argv[8]));
    pinpong2(argv, zz);
}
